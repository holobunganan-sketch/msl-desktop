...
if '--layout' in sys.argv:
    ...
    current=next(w for w in call('list_works',status=None) if w['id']==sample['id'])
    call('delete_work',id=current['id'],confirmationName=current['title'],expectedRevision=current['revision']); refresh()
...
current=next(w for w in call('list_works',status=None) if w['id']==a['id'])
call('delete_work',id=current['id'],confirmationName=current['title'],expectedRevision=current['revision']); refresh()
