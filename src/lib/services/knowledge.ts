...
export type Answer={claims:{text:string;basis:string;citations:Citation[]}[];gaps:string[]};
export type Session={id:number;title:string;scope_json:string;updated_at:number};
export type Turn={id:number;session_id:number;question:string;scope_json:string;status:string;answer_json:string|null;evidence_json:string|null;error:string|null;created_at:number};
