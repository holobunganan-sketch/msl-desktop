<script lang="ts">
  ...
  let {item,works,compact=false}: {item:{kind:string;operation?:string;work_id:number|null;payload_json:string;reason:string};works:Array<{id:number;title:string}>;compact?:boolean}=$props();
...
<div class="proposal-preview" data-testid="proposal-preview">
  ...
  {#if compact}
    {#if preview.changes.length || item.reason}<details data-testid="proposal-details"><summary>{$locale==='en-US'?'Arrangement details':'查看安排详情'}</summary>{#if preview.changes.length}<ul>{#each preview.changes as change}<li>{change}</li>{/each}</ul>{/if}{#if item.reason}<p>{item.reason}</p>{/if}</details>{/if}
  {:else}
