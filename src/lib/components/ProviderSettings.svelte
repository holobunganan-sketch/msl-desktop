<script lang="ts">
  ...
  function fold(id:number,open:boolean){if(expanded[id]===open)return;expanded={...expanded,[id]:open};try{localStorage.setItem('msl.provider-model-folds',JSON.stringify(expanded));}catch{}}
  ...
  async function load() {
    ...
    try {
      ...
      await Promise.all(connections.map(async connection=>{
        [nextModels[connection.id],nextKeys[connection.id]]=await Promise.all([command<ProviderModel[]>("list_provider_models",{providerId:connection.id}),command<boolean>("provider_has_key",{id:connection.id})]);
      }));
  ...
  async function refresh(connection: ProviderConnection) {
    ...
    try { await command("refresh_provider_models", { providerId: connection.id }); addToast(tt("settings.modelsRefreshed"), "success"); models={...models,[connection.id]:await command<ProviderModel[]>("list_provider_models",{providerId:connection.id})}; }
    catch (value) { error = normalizeError(value); addToast(error, "error"); }
    finally { busy = null; }
  }
  ...
  onMount(()=>{try{const saved=JSON.parse(localStorage.getItem('msl.provider-model-folds')??'{}');if(saved&&typeof saved==='object')expanded=Object.fromEntries(Object.entries(saved).filter(([,v])=>typeof v==='boolean')) as Record<number,boolean>;}catch{}void load();});
</script>
...
<section class="provider-area">
  ...
  <div class="provider-status" aria-live="polite">{#if error}<button class="error" onclick={()=>showError=true} title={error}>{error}</button>{:else if loading}<span>{tt("common.loading")}</span>{:else if connections.length===0}<span>{tt("settings.noProviders")}</span>{/if}</div>
  <Modal bind:open={showError} title={currentLocale==="zh-CN"?"错误详情":"Error details"}><p class="error-detail">{error}</p></Modal>
...
.provider-status{height:32px;min-width:0;display:flex;align-items:center;font-size:14px;color:var(--color-muted)}.provider-status .error{max-width:100%;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;cursor:pointer}.error-detail{white-space:pre-wrap;overflow-wrap:anywhere}
 .model-list{margin-top:16px;border-top:1px solid var(--color-border)}.model-list summary{padding:14px 4px;cursor:pointer;color:var(--color-muted);font-size:14px;min-height:44px;user-select:none}.model-grid{margin-top:0}.model-row{display:grid;grid-template-columns:minmax(0,1fr) 340px;min-height:64px}.model-row>div:first-child{min-width:0;overflow-wrap:anywhere}.model-status{display:grid;grid-template-columns:54px 54px 108px 100px;gap:6px;flex-wrap:nowrap}.model-status .tag{margin:0;justify-content:center}.model-status :global(.app-button){width:100%;font-size:13px}.connection-head>div:first-child{min-width:0;overflow-wrap:anywhere}.connection-key{grid-template-columns:minmax(0,1fr) 150px minmax(160px,.8fr)}
 @media(max-width:950px){.model-row{grid-template-columns:minmax(0,1fr)}.model-status{justify-content:start}.connection-key{grid-template-columns:minmax(0,1fr) 150px}.connection-key small{grid-column:1/-1}}
</style>
