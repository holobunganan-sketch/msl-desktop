<script lang="ts">
 ...
 let canSend=$derived(ready&&!sessionLoading&&modelReady&&!busy&&!processing&&!savingModel&&!!question.trim());
 ...
 function remember(){if(!ready)return;if(sessionId&&transcript&&!sessionLoading&&!scrollRestoring)scrollPositions.set(sessionId,{top:transcript.scrollTop,follow:followBottom});chatDrafts.set(String(sessionId??'new'),{question,scope:[...scope]});chatSelection.sessionId=sessionId;}
 function restore(id:number|null,defaultScope:number[]){const draft=chatDrafts.get(String(id??'new'));question=draft?.question??'';scope=[...(draft?.scope??defaultScope)];caret=question.length;picker=false;suppressMention=true;highlight=0;followBottom=id===null?true:(scrollPositions.get(id)?.follow??true);}
 ...
 async function select(s:Session){
  ...
  const epoch=++sessionEpoch;scrollRestoring=true;sessionId=s.id;sessionLoading=true;restore(s.id,json(s.scope_json,[]));turns=[];error='';removeOpen=false;historyOpen=false;
 ...
 function fresh(){remember();sessionEpoch++;scrollRestoring=false;sessionLoading=false;sessionId=null;restore(null,[]);turns=[];error='';removeOpen=false;historyOpen=false;void tick().then(()=>textarea?.focus());}
 ...
 onMount(()=>{let active=true;void (async()=>{try{await Promise.all([listWorks().then(p=>projects=p),loadModels(),refresh()]);if(!active)return;ready=true;if(workId){scope=[workId];}else{const chosen=focusId??chatSelection.sessionId;const s=chosen===null?undefined:sessions.find(s=>s.id===chosen)??sessions[0];if(s){await select(s);}else restore(null,[]);}ready=true;}catch(e){error=normalizeError(e);}})();const timer=setInterval(()=>{void refresh().catch(()=>{});},2200);return()=>{remember();active=false;disposed=true;sessionEpoch++;clearInterval(timer);};});
</script>
  ...
  <main class="qa-main">
   ...
   <div class="qa-composer-wrap" data-testid="qa-composer">
    ...
    <div class="qa-composer-box">
     <div class="qa-scope-tags" data-testid="qa-scope">{#each scope as id(id)}<button title={scopeName([id])} onclick={()=>scope=scope.filter(x=>x!==id)} aria-label={(en?'Remove focus: ':'移除聚焦：')+scopeName([id])}>@ {scopeName([id])} ×</button>{/each}{#if scope.length}<button onclick={()=>scope=[]}>{en?'All information':'恢复全局'}</button>{:else}<small>{en?'Searching all workbench information':'当前范围：全部工作台信息'}</small>{/if}</div>
