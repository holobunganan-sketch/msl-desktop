...
<section class="routing"><div class="section-head"><div><h2>{tt("settings.aiRouting")}</h2><p>{tt("settings.aiRoutingHint")}</p></div></div><div class="error stable-feedback"><StatusLine message={error}/></div><div class="route-list">{#each taskKinds as kind}<div class="route-row"><div><strong>{tt(taskLabel[kind] as Parameters<typeof t>[0])}</strong><small>{modelLabel(routeFor(kind))}</small></div><select value={routeFor(kind) ?? ""} onchange={(event) => save(kind, (event.currentTarget as HTMLSelectElement).value)} disabled={saving === kind} aria-label={tt(taskLabel[kind] as Parameters<typeof t>[0])}><option value="">{tt("settings.routeUnconfigured")}</option>{#each modelOptions() as model}<option value={model.id}>{modelLabel(model.id)} · {model.protocol}{model.available ? "" : ` · ${tt("settings.unavailable")}`}</option>{/each}</select><span class="muted route-saving">{saving === kind ? tt("common.loading") : ""}</span></div>{/each}</div></section>

<style>
  .routing { display:grid; gap:10px; } .section-head h2 { margin:0 0 4px; font-size:14px; } .section-head p { margin:0; color:var(--color-muted); font-size:10px; } .route-list { display:grid; gap:2px; } .route-row { display:grid; grid-template-columns:minmax(0,1fr) minmax(220px,360px) 72px; align-items:center; gap:10px; border-top:1px solid var(--color-border); padding:9px 0; } .route-row strong{font-size:10px}.route-row small { display:block; color:var(--color-muted); font-size:8px; margin-top:3px; } select { width:100%; border:1px solid var(--color-border); border-radius:var(--radius-sm); padding:8px; background:#fbfcfc; color:var(--color-text);font-size:9px } .muted { color:var(--color-muted); font-size:9px; } .error { padding:8px 10px; border:1px solid #ead2d2; background:var(--color-danger-soft); color:var(--color-danger); border-radius:var(--radius-sm); font-size:10px; } @media(max-width:720px){ .route-row { grid-template-columns:1fr; } }
