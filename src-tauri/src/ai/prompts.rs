pub const PROMPT_VERSION: &str = "msl-secretary-v8-open-evidence";
...
<action_boundary>Perform translation only. Do not take actions or interpret work records.</action_boundary>"#
    ...
    let task = match stage {
        SecretaryStage::DailyBrief => "Create a concise cross-source management brief for the selected period. Analyze horizontally across workspaces and document contents, Work, Task, Waiting, Calendar, Inbox and Resume Point; connect records that concern the same workstream and identify conflicts or dependencies. Analyze vertically from past changes and completed work, through current state and blockers, to the next practical action. Cover past progress, work in progress, today's calendar, open or overdue tasks, waiting or blocked items, inbox items and workspace/file changes. Return concise factual lines covering all distinct useful findings. Every non-empty line must start with the bullet character '•'. The display summary must not contain source_type, entity_id, workspace_id, source_id or bracketed technical references; evidence remains in the structured source data. Do not use paragraphs, headings, tables or markdown fences. Prioritize readability and relevance without discarding supported findings solely to meet a fixed item count.",
